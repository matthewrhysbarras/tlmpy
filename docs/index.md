# TLMpy

TLMpy is an experimental Python package for scalar 2D TLM-style wave simulation and diffusion validation references.

- [Getting started](getting_started.md)
- [Numerical methods](numerical_methods.md)
- [Validation](validation.md)
- [Benchmarking](benchmarking.md)
- [Boundary characterisation](boundary_characterisation.md)
- [Dispersion characterisation](dispersion_characterisation.md)
- [Example gallery](example_gallery.md)
- [TLM reference inventory](reference_inventory.md)
- [Roadmap](roadmap.md)

Design notes:

- [Benchmarking against references](design/benchmarking_against_references.md)
- [Meep comparison feasibility](design/meep_comparison_feasibility.md)
- [Python fdtd comparison feasibility](design/fdtd_comparison_feasibility.md)
